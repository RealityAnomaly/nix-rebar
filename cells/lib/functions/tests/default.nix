...
in {
  ...
  loadNixt = tests: let
    convertCases = lib.mapAttrs (_: lib.mapAttrs (_k: v: v.expr == v.expected));
  in nixt.grow {
    blocks = lib.mapAttrsToList (
      _: tests: nixt.block' (/. + tests.path) (convertCases tests.value.unit)
    ) (lib.filterAttrs (_: v: v.value ? "unit") tests);
  };

  loadNixpkgs = tests: lib.mapAttrsToList (
    _: tests: lib.mapAttrs (_: cases: lib.debug.runTests (cases // {
      tests = lib.attrNames cases;
    })) tests.value.unit
  ) (lib.filterAttrs (_: v: v.value ? "unit") tests);
}
